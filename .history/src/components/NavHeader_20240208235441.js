...
function NavHeader({ userId, handleProductClick, cartunmber }) {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      setNotificationsNumber(response.data.data.notification);
      console.log(Number(response.data.data.notification));
    } catch (error) {
            ...
            <div className="flexNavone">
              ...
              <div className="flexRightNav">
                ...
                <div className="text-line text-linelogout">
                  {isLoggedIn && (
                    <>
                      <div onClick={handleNotificationsClick} className="outline-dashed">
                        <IoIosNotificationsOutline
                          className="noteicon"
                          
                        />
                        <div className=" lg:top-9 top-25 w-10 h-10 rounded-full bg-red-500 absolute text-center items-center py-1 text-white opacity-70">
                          {length(notificatonsNumber.length)}
                        </div>
                      </div>
